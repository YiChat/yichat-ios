...
- (void)appDidBeginActive:(NSNotification *)notfy{
    ...
    if (self.isPay) {
        if (self.out_trade_no && self.out_trade_no.length > 0) {
            [YiChatRedPacketHelper queryTradeNo:self.out_trade_no status:^(BOOL status) {
                weakSelf.isPay = NO;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cell == nil) {
        ...
        whiteBG.backgroundColor = PROJECT_COLOR_APPBACKCOLOR;
...
-(UIView *)tabFootView{
    ...
    textView.backgroundColor = PROJECT_COLOR_APPBACKCOLOR;
...
-(void)payRedPacket:(NSString *)passWord type:(NSString *)type{
    ...
    if (weakSelf.isGroup) {
        param = [ProjectRequestParameterModel getSendGroupRedPacketParametersWithGroupId:weakSelf.chatId money:weakSelf.redMoney content:weakSelf.redContent nun:weakSelf.redNum password:passWord type:type];
        
    }else{
        param = [ProjectRequestParameterModel getSendSingleRedPacketParametersWithReceiveUserId:weakSelf.chatId money:weakSelf.redMoney content:weakSelf.redContent password:passWord type:type];
        
    }
    
    [YiChatRedPacketHelper sendRedPacketGroup:weakSelf.isGroup param:param successDic:^(NSDictionary * _Nonnull successDic) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
-(void)reloadBalance{
    ...
    [YiChatRedPacketHelper searchBalance:^(NSString * _Nonnull balance) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self.balance < self.redMoney.floatValue) {
                    ...
                    UIAlertAction *aliPay = [UIAlertAction actionWithTitle:@"支付宝支付" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                        [YiChatRedPacketHelper aliPayType:@"1" money:self.redMoney payBlock:^(BOOL isInstallation, NSString * _Nonnull out_trade_no) {
                            if (isInstallation){
                                weakSelf.isPay = YES;
                    ...
                    UIAlertAction *wechatPay = [UIAlertAction actionWithTitle:@"微信支付" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                        [YiChatRedPacketHelper weChatPayType:@"1" money:self.redMoney payBlock:^(BOOL isInstallation, NSString * _Nonnull out_trade_no) {
                            if (isInstallation){
...
-(void)showChossesPayView:(NSString *)balance{
    ...
    [weakSelf.popView showAtCenter:CGPointMake(self.view.frame.size.width / 2, self.view.frame.size.height/ 2 - 100) inView:self.view];
}
