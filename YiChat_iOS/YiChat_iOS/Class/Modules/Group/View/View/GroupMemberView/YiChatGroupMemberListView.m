                ...
                if(weakSelf.yiChatGroupMemberListViewFetchUserPower){
                    ...
                    if(power > 0){
                        ...
                        if(selecte && [selecte isKindOfClass:[NSIndexPath class]]){
                            if(weakSelf.dataSoureArr.count - 1 >= selecte.row){
                                id model = weakSelf.dataSoureArr[selecte.row];
                                
                                YiChatUserModel *user = model;
                                
                                NSString *userId = user.getUserIdStr;
                                
                                YiChatGroupInfoModel *groupM = weakSelf.YiChatGroupMemberInfo();
                                ...
                                NSString *string = [NSString stringWithFormat:@"%@",groupM.owner];
//                                if ([groupM.adminList containsObject:userId] || [string isEqualToString:userId]) {
//                                    isAdd = YES;
//                                }
                                ...
                                if (isAdd) {
                                    if(model && [model isKindOfClass:[YiChatUserModel class]]){
                                        if(weakSelf.yiChatGroupMemberListViewClickItems){
                                            weakSelf.yiChatGroupMemberListViewClickItems(model);
                                        }
                                    }
                                }
