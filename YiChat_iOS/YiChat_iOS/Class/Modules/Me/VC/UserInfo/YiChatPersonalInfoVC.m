        ...
        for (int i = 0; i < self.sectionsRowsNumSet.count; i ++) {
            ...
            for (int j = 0; j <num.integerValue ; j ++) {
                ...
                if(i == 0 && j == 0){
                    ...
                    model.contentStr = [modelUser ucodeStr];
                }
                else if(i == 1 && j == 2){
                    model.contentStr = [modelUser recommendCodeStr];
                }
            }
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([dataModel.titleStr isEqualToString:@"头像"] || [dataModel.titleStr isEqualToString:@"我的二维码"]){
        ...
        BOOL arrow = YES;
        if ([dataModel.titleStr isEqualToString:@"我的推广码"] || [dataModel.titleStr isEqualToString:@"我的邀请码"]) {
            arrow = NO;
        }else{
            arrow = YES;
        }
        
        [cell updateSystemConfigWithIndexPath:indexPath arrow:[NSNumber numberWithBool:arrow] downLine:[NSNumber numberWithBool:YES]  cellHeight:[NSNumber numberWithFloat:cellH]];
