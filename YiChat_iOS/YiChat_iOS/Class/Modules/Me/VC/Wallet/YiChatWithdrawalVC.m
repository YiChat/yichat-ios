...
-(void)setUI{
    ...
    addBankView.backgroundColor = PROJECT_COLOR_APPBACKCOLOR;
...
//    }
    ...
    self.withdrawalBtn = withdrawalBtn;
        ...
        [ProjectRequestHelper requestHelper_feltRequestData:data response:response handle:^(id  _Nonnull obj, BOOL isNeedLogin) {
            if([obj isKindOfClass:[NSDictionary class]]){
                ...
                if (model.code == 0) {
                    ...
                    NSString *openStatus = [NSString stringWithFormat:@"%@",dic[@"openStatus"]];
                    
                    weakSelf.rate = rateStr.floatValue;
                    weakSelf.minLimit = minLimitStr.floatValue;
                    dispatch_async(dispatch_get_main_queue(), ^{
                        weakSelf.textView.text = text;
                        if (![openStatus isEqualToString:@"0"]) {
                            weakSelf.withdrawalBtn.backgroundColor = [UIColor grayColor];
                            weakSelf.withdrawalBtn.userInteractionEnabled = NO;
                        }
                    });
                }
