...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *dic = self.dataArr[indexPath.row];
    YiChatWebVC *vc = [YiChatWebVC initialVC:[NSString stringWithFormat:@"%@",dic[@"name"]]];
    vc.url = [NSString stringWithFormat:@"%@",dic[@"memo"]];
...
//    NSDictionary *param = [ProjectRequestParameterModel getSetPayPassWord:self.passWord1];
    ...
    WS(weakSelf);
    [ProjectRequestHelper userDepositItem:@{} headerParameters:[ProjectRequestParameterModel getTokenParamWithToken:YiChatUserInfo_Token] progress:nil isScrete:YES isAsyn:YES identider:^(NSString * _Nonnull identify) {
