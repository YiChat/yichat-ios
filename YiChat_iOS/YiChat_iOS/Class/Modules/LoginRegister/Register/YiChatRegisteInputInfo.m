...
@property (nonatomic,strong) UITextField *recommendCode;
@end
...
- (void)submit{
    ...
    [self.recommendCode resignFirstResponder];
...
//        [ProjectUIHelper ProjectUIHelper_getAlertWithMsm:@"请输入邀请码"];
//        return;
//    }
    NSString *platFormStr = @"0";
    NSString *recommendCode = @"";
    if (self.recommendCode.text.length > 0) {
        recommendCode = self.recommendCode.text;
    }
    NSDictionary *parameters = [ProjectRequestParameterModel getRegisterParametersWithMobile:_phoneNumStr nick:_user_nick.text password:_user_password.text avrater:_user_Icon_Str platform:platFormStr recommendCode:recommendCode];
        ...
        [ProjectRequestHelper requestHelper_feltRequestData:data response:response handle:^(id  _Nonnull obj, BOOL isNeedLogin) {
            ...
            if([res isKindOfClass:[NSDictionary class]]){
                ...
                if([data isKindOfClass:[NSDictionary class]]){
                    ...
                    NSString *recUserId = [NSString stringWithFormat:@"%@",dic[@"recUserId"]];
                    [[NSUserDefaults standardUserDefaults] setObject:recUserId forKey:@"recUserId"];
...
- (UIView *)inputViews{
    if(!_inputViews){
          _inputViews = [[UIView alloc] initWithFrame:CGRectMake(0, self.topView.frame.origin.y + self.topView.frame.size.height + 20.0, PROJECT_SIZE_WIDTH, 180.0)];
        CGFloat blank = PROJECT_SIZE_NAV_BLANK;
        
        NSArray *placeholders = @[@"昵称（必填）",@"密码（必填）",@"邀请码(选填)"];
        for (int i = 0; i < 3; i ++) {
            NSString *str = nil;
