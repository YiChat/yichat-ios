...
+ (id)initialVC{
    YiChatConversationVC *conversation = [YiChatConversationVC initialVCWithNavBarStyle:ProjectNavBarStyleCommon_9 centeritem:PROJECT_TEXT_LOCALIZE_NAME(@"conversationMain") leftItem:nil rightItem:[UIImage imageNamed:@"add_new.png"]];
    return conversation;
}
...
- (void)viewDidLoad {
    ...
    [ProjectRequestHelper configLinkWithParameters:@{} headerParameters:[ProjectRequestParameterModel getTokenParamWithToken:YiChatUserInfo_Token] progress:nil isScrete:YES isAsyn:YES identider:^(NSString * _Nonnull identify) {
        ...
        [ProjectRequestHelper requestHelper_feltRequestData:data response:response handle:^(id  _Nonnull obj, BOOL isNeedLogin) {
            if([obj isKindOfClass:[NSDictionary class]]){
                ...
                if (model.code == 0) {
                    NSString *url = [NSString stringWithFormat:@"%@",dataDic[@"data"]];
                    [[NSUserDefaults standardUserDefaults] setObject:url forKey:@"configLink"];
...
//    self.view.backgroundColor = [UIColor colorWithRed:106 / 255.0 green:187 / 255.0 blue:185 / 255.0 alpha:1];
...
//         [ProjectRequestHelper requestHelper_feltRequestData:data response:response handle:^(id  _Nonnull obj, BOOL isNeedLogin) {
////             if ([obj isKindOfClass:[NSDictionary class]]) {
//                 NSString *data = [NSString stringWithFormat:@"%@",obj[@"data"]];
//                 NSLog(@"%@",data);
////             }
...
//    self.cTable.backgroundColor = [UIColor colorWithRed:106 / 255.0 green:187 / 255.0 blue:185 / 255.0 alpha:1];
                ...
                if(model && [model isKindOfClass:[HTConversation class]]){
                    if ([[NSUserDefaults standardUserDefaults] stringForKey:placedTop]) {
                        ...
                        if ([s isEqualToString:model.chatterId]) {
                            ...
                            cell.backgroundColor = PROJECT_COLOR_APPBACKCOLOR;
                        }
                    }else{
                        cell.backgroundColor = PROJECT_COLOR_APPBACKCOLOR;
                    }
                    ...
                    if([model.lastMessage.chatType isEqualToString:@"1"]){
                        ...
                        [[YiChatUserManager defaultManagaer] connectionLoadInvocation:^(YiChatConnectionModel * _Nonnull connectionModel, NSString * _Nonnull error) {
                            ...
                            if(connectionModel && [connectionModel isKindOfClass:[YiChatConnectionModel class]]){
                                ...
                                NSString *type = model.lastMessage.chatType;
                                
                                HTConversation *m = model;
                                if(isHas){
                                    ...
                                    [[YiChatUserManager defaultManagaer] judgeFriendshipWithFriendId:chatId invocation:^(NSString * _Nonnull frinedShip) {
                                        if ([frinedShip isEqualToString:@"1"]) {
                                            [[YiChatUserManager defaultManagaer] updateUserConnectionInvocation:^(YiChatConnectionModel * _Nonnull model, NSString * _Nonnull error) {
                                                [ProjectHelper helper_getMainThread:^{
                                                    UIViewController *chat = [ZFChatUIHelper getChatVCWithChatId:chatId chatType:type];
                                                    chat.hidesBottomBarWhenPushed = YES;
                                                    [self.navigationController pushViewController:chat animated:YES];
                                                    
                                                    [ZFChatHelper zfCahtHelper_updateLocalConcersationWithConversation:m isReadAllMessage:YES];
                                                }];
                                            }];
                                        }
                                    }];
                                    
                                    [[YiChatUserManager defaultManagaer] fetchUserInfoWithUserId:chatId invocation:^(YiChatUserModel * _Nonnull model, NSString * _Nonnull error) {
                                        if(model && [model isKindOfClass:[YiChatUserModel class]]){
