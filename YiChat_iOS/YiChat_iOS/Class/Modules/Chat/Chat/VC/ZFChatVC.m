            ...
            if(userId && [userId isKindOfClass:[NSString class]]){
                ...
                if(weakSelf.presenter.getChatType == ZFChatTypeGroup){
                    if(weakSelf.presenter.getPower > 0)
                    {
                        ...
                        if (YiChatProject_IsAddGroupMember == 0) {
                            return;
                        }
                        
                        NSString *string = [NSString stringWithFormat:@"%@",weakSelf.presenter.chatGroupInfoModel.owner];
                        BOOL isAdd = NO;
                        
                        NSMutableArray *arr = [NSMutableArray new];
                        for (NSDictionary *dic in weakSelf.presenter.chatGroupInfoModel.adminList) {
                            NSString *uid = [NSString stringWithFormat:@"%@",dic[@"userId"]];
                            [arr addObject:uid];
                            
                        }
                        
                        if (YiChatProject_IsAddGroupMember == 1) {
                            if ([string isEqualToString:userId]) {
                                isAdd = YES;
                            }
                        }
                        
                        if (YiChatProject_IsAddGroupMember == 2) {
                            if ([arr containsObject:userId] || [string isEqualToString:userId]) {
                                isAdd = YES;
                            }
                        }
                        
                        if (YiChatProject_IsAddGroupMember == 3) {
                            isAdd = YES;
                        }
                        
                        if (isAdd) {
                            NSIndexPath *selecte = index;
                            if(selecte && [selecte isKindOfClass:[NSIndexPath class]]){
                                ZFChatConfigure *chat = [weakSelf.presenter getChatDataWithIndex:selecte.section];
                                if(chat && [chat isKindOfClass:[ZFChatConfigure class]]){
