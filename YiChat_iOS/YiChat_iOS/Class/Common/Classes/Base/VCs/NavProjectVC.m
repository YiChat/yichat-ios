...
- (void)changeWhiteNavStyle{
    UILabel *left = _cLeftItem;
//    left.image = [UIImage imageNamed:@"icon_back_w@3x.png"];
    left.textColor = [UIColor whiteColor];
    for (id control in _cBackView.subviews) {
        if(control && [control isKindOfClass:[UILabel class]]){
            UILabel *lab = control;
