...
-(void)fixedTime{
    ...
    dispatch_source_set_timer(self.timer, DISPATCH_TIME_NOW, 300 * NSEC_PER_SEC, 1 * NSEC_PER_SEC);
    dispatch_source_set_event_handler(self.timer, ^{
        [weakSelf userOnlineRecord];
    });
    dispatch_resume(self.timer);
}

-(void)userOnlineRecord{
    [ProjectRequestHelper userOnlineRecordWithParameters:@{} headerParameters:[ProjectRequestParameterModel getTokenParamWithToken:YiChatUserInfo_Token] progress:nil isScrete:YES isAsyn:YES identider:^(NSString * _Nonnull identify) {
...
- (void)selecteIndex:(NSInteger)index{
    ...
    if (index == 2 && self.selectedIndex == 2) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"SELECTETAB" object:nil];
        [[NSNotificationCenter defaultCenter] removeObserver:self name:@"SELECTETAB" object:nil];
    }
...
- (void)adjustTabbarSubView{
    ...
    CGFloat text_w_single = self.tabBar.frame.size.width / _tagsArr.count;
    CGFloat iconNumY = 0;
    for (int i = 0; i < _tagsArr.count; i ++ ) {
        ...
        indexLab.frame = CGRectMake(x + text_w_single * i, y, self.tabBar.frame.size.width / _tagsArr.count , h);
