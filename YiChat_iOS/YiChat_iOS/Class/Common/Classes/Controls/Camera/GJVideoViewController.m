...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error{
    ...
    if (self.isVideo) {
        ...
        if (!self.player) {
            self.player = [[GJAVPlayer alloc] initWithFrame:self.view.frame withShowInView:self.bgView url:outputFileURL];
        } else {
            if (outputFileURL) {
...
- (void)videoHandlePhoto:(NSURL *)url {
    ...
    if(self.takeImage){
        CGFloat h = self.view.frame.size.height;
        CGFloat w = [ProjectHelper helper_GetWidthOrHeightIntoScale:self.takeImage.size.width / self.takeImage.size.height width:0 height:h];
        self.takeImageView.frame = CGRectMake(self.view.frame.size.width / 2 - w / 2, self.view.frame.size.height / 2 - h / 2, w, h);
    }
