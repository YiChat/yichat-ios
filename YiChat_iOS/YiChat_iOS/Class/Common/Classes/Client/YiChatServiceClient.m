...
-(void)sendFriendMsg:(NSString *)applyUserId{
...
//                if([status isEqualToString:@"0"]){
                ...
                [ext addEntriesFromDictionary:dic];
                [ext addEntriesFromDictionary:@{@"action":@"50001"}];
                HTMessage *msg = [ZFChatMessageHelper sendTextMessage:@"我们已经成为好友了快来聊天吧～" to:applyUserId messageType:@"1" messageExt:ext];
                
                [[HTClient sharedInstance] sendMessage:msg completion:^(HTMessage *message, NSError *error) {
                    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"recUserId"];
                    [[NSUserDefaults standardUserDefaults] synchronize];
                }];
                
//                [ZFChatFriendHelper zfChatFriendHelperAgreeFriendApplyWithUserId:applyUserId userInfo:dic completion:^(HTCmdMessage * _Nonnull msg, NSError * _Nonnull err) {
////                    if (msg) {
//                    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"recUserId"];
//                    [[NSUserDefaults standardUserDefaults] synchronize];
////                    }
...
- (void)yiChatServiceClient_loginWithUserName:(NSString *)userName password:(NSString *)password invocation:(void(^)(BOOL state))invocation{
    WS(weakSelf);
    [ZFChatHelper zfChatHelper_loginWithUserName:userName password:password completion:^(BOOL success, NSString * _Nonnull des) {
        [ProjectHelper helper_getMainThread:^{
            ...
            if(success){
                ...
                if ([[NSUserDefaults standardUserDefaults] objectForKey:@"recUserId"]) {
                    NSString *recUserId = [[NSUserDefaults standardUserDefaults] objectForKey:@"recUserId"];
                    [weakSelf sendFriendMsg:recUserId];
                }
            }
            else{
                weakSelf.xmppIsLogin = YES;
